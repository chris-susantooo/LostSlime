...
export default class TitleScene extends Scene {
    ...
    loadVisualAssets() {
        ...
        loadImage('/img/title/Menu.png').then(image => {
            let menu = new Entity(calScaledMid(image, canvas, 0, -400), new Vec2(0, 0), image);
            this.addEntity('menu', menu, 2);
        });
        //buttons
        loadImage('/img/title/pvp button.png').then(image => {
            let pvp = new Entity(calScaledMid(image, canvas, 0, -250), new Vec2(0, 0), image);
        ...
        loadImage('/img/title/survival button.png').then(image => {
            let survival = new Entity(calScaledMid(image, canvas, 0, -600), new Vec2(0, 0), image);
        ...
        loadImage('/img/title/title_1.png').then(image => {
            let title = new Entity(calScaledMid(image, canvas, 0, 550), new Vec2(0, 0), image);
            this.addEntity('title', title, 3);
        });
        //slimes
        loadImage('/img/title/char1.png').then(image => {
            let yellowSlime = new Entity(calScaledMid(image, canvas, 500, 800), new Vec2(0, 0), image);
            this.addEntity('yellowSlime', yellowSlime, 2);
        });
        loadImage('/img/title/char2.png').then(image => {
            let pinkSlime = new Entity(calScaledMid(image, canvas, 1700, -800), new Vec2(0, 0), image);
        ...
        loadImage('/img/title/char3.png').then(image => {
            let greenSlime = new Entity(calScaledMid(image, canvas, -500, -800), new Vec2(0, 0), image);
            this.addEntity('greenSlime', greenSlime, 2);
        });
        loadImage('/img/title/char4.png').then(image => {
            let blueSlime = new Entity(calScaledMid(image, canvas, -1200, 200), new Vec2(0, 0), image);
